            ...
            .addOnSuccessListener { result ->
                ...
                result.documents.forEach { doc ->
                    ...
                    menuOrders?.forEach { (menuName, menuAmount) ->
                        val current = menuSalesMap[menuName.toString()] ?: 0
                        menuSalesMap[menuName.toString()] = current + (menuAmount as? Long)?.toInt()!!
                    }
                }

                // 오늘 총 매출
                binding.tvTotalSales.text = "${String.format("%,d", totalSales)}원"
                ...
                menuSalesMap.entries.sortedByDescending { it.value }.forEach { (name, amount) ->
                    ...
                    row.findViewById<TextView>(android.R.id.text2).text = "${String.format("%,d", amount)}원"
    ...
    private fun loadWeeklySales(uid: String) {
        ...
        db.collection("stores").document(uid)
            ...
            .addOnSuccessListener { result ->
                ...
                binding.tvTotalWeek.text = "총 ${String.format("%,d", totalWeek)}원"
                binding.tvDailyAverage.text = "일평균 ${String.format("%,d", average)}원"
            }
    }
